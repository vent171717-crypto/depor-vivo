...
<title>FILMAX - TV NAVIGATION</title><style>
...
.logo{color:var(--primary);font-weight:900;font-size:28px;letter-spacing:-1px;text-transform:uppercase;cursor:pointer;transition:all 0.2s}
.logo:focus{outline:none;background:rgba(229,9,20,0.2);padding:5px 10px;border-radius:4px}
...
.nav-item{padding:10px 20px;color:var(--light2);font-size:14px;cursor:pointer;transition:all 0.3s;display:flex;align-items:center;gap:10px;border-left:3px solid transparent}
.nav-item:hover,.nav-item.focused{color:var(--light);background:rgba(229,9,20,0.15);border-left-color:var(--primary);padding-left:17px}
...
.search-input{width:100%;background:rgba(255,255,255,0.1);border:2px solid rgba(255,255,255,0.2);color:var(--light);padding:10px 15px;border-radius:4px;font-size:14px;transition:all 0.3s;outline:none}
.search-input:focus,.search-input.focused{border-color:var(--primary);background:rgba(255,255,255,0.15);box-shadow:0 0 15px rgba(229,9,20,0.3)}
.search-input::placeholder{color:var(--light2)}
.user-profile{width:40px;height:40px;background:var(--primary);border-radius:4px;display:flex;align-items:center;justify-content:center;font-weight:700;cursor:pointer;transition:all 0.3s;border:2px solid transparent}
.user-profile:focus,.user-profile.focused{outline:none;border-color:var(--light);transform:scale(1.15);box-shadow:0 0 15px rgba(229,9,20,0.5)}
...
.hero-title{font-size:48px;font-weight:900;margin-bottom:15px;text-transform:uppercase;letter-spacing:-1px;text-shadow:2px 2px 4px rgba(0,0,0,0.8)}
...
.btn-play,.btn-info{padding:12px 30px;border:2px solid transparent;border-radius:4px;font-size:15px;font-weight:600;cursor:pointer;transition:all 0.3s;display:flex;align-items:center;gap:8px;outline:none}
.btn-play{background:var(--light);color:#000}
.btn-play:hover,.btn-play.focused{background:#e0e0e0;transform:scale(1.08);box-shadow:0 0 20px rgba(229,9,20,0.5);border-color:var(--primary)}
.btn-info{background:rgba(109,109,110,0.7);color:var(--light)}
.btn-info:hover,.btn-info.focused{background:var(--primary);color:#000;transform:scale(1.08);box-shadow:0 0 20px rgba(229,9,20,0.5);border-color:var(--primary)}
...
.card{position:relative;aspect-ratio:2/3;background:var(--dark2);border-radius:6px;overflow:hidden;cursor:pointer;transition:all 0.3s;border:2px solid transparent;flex-shrink:0;outline:none}
.card:hover,.card.focused{transform:scale(1.12);box-shadow:0 8px 25px rgba(229,9,20,0.6);border-color:var(--primary);z-index:20}
...
.card:hover .card-overlay,.card.focused .card-overlay{opacity:1}
.card-title{font-size:12px;font-weight:600;line-height:1.4}
.card-play{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:40px;opacity:0;transition:opacity 0.2s}
.card.focused .card-play{opacity:1}
...
.nav-hint{position:fixed;bottom:10px;left:30px;background:rgba(0,0,0,0.8);color:var(--light2);padding:12px 16px;border-radius:4px;font-size:11px;border-left:3px solid var(--primary);z-index:50;max-width:300px}
...
.p-back{background:rgba(0,0,0,0.5);border:2px solid rgba(255,255,255,0.2);color:var(--light);font-size:24px;cursor:pointer;width:44px;height:44px;display:flex;align-items:center;justify-content:center;border-radius:4px;transition:all 0.3s;outline:none}
.p-back:hover,.p-back:focus,.p-back.focused{background:var(--primary);color:#000;border-color:var(--primary);transform:scale(1.15);box-shadow:0 0 15px rgba(229,9,20,0.6)}
.p-title{font-size:18px;font-weight:700;text-shadow:2px 2px 4px rgba(0,0,0,0.8)}
.p-bottom{padding:20px;padding-bottom:max(20px,env(safe-area-inset-bottom))}
.p-prog{display:flex;align-items:center;gap:12px;margin-bottom:15px;cursor:pointer}
.p-time{font-size:12px;min-width:50px;font-weight:600}
.p-bar{flex:1;height:8px;background:rgba(255,255,255,0.2);border-radius:4px;position:relative;cursor:pointer;transition:background 0.3s}
.p-bar.focused{background:rgba(255,255,255,0.4);height:12px}
...
.p-btn{width:50px;height:50px;background:rgba(255,255,255,.1);border:2px solid rgba(255,255,255,.2);border-radius:50%;color:var(--light);font-size:14px;cursor:pointer;transition:all 0.3s;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px);outline:none;font-weight:700}
.p-btn:hover,.p-btn:focus,.p-btn.focused{background:rgba(229,9,20,0.5);border-color:var(--primary);transform:scale(1.2);box-shadow:0 0 20px rgba(229,9,20,0.6)}
.p-btn.main{width:60px;height:60px;font-size:20px;background:var(--primary);color:#000;border:none}
.p-btn.main:hover,.p-btn.main:focus,.p-btn.main.focused{background:#ff1a1a;transform:scale(1.3);box-shadow:0 0 30px rgba(229,9,20,0.8)}
...
<div id="app">
    <div class="sidebar" id="sidebar">
        <div class="logo-box">
            <div class="logo" id="logo" tabindex="0">► Filmax</div>
        </div>
        <div class="nav-section">
            <div class="nav-title">Menú</div>
            <div class="nav-item focused" data-nav="home" tabindex="0"><span class="nav-icon">🏠</span> Inicio</div>
            <div class="nav-item" data-nav="trending" tabindex="0"><span class="nav-icon">🔥</span> Tendencias</div>
            <div class="nav-item" data-nav="search" tabindex="0"><span class="nav-icon">🔍</span> Buscar</div>
        </div>
        <div class="nav-section">
            <div class="nav-title">Mi Lista</div>
            <div class="nav-item" tabindex="0"><span class="nav-icon">❤️</span> Favoritos</div>
            <div class="nav-item" tabindex="0"><span class="nav-icon">👁️</span> Viendo</div>
        </div>
    </div>
    ...
    <div class="main-content">
        <div class="top-bar">
            <div class="search-box">
                <input class="search-input" id="srch" placeholder="Buscar..." autocomplete="off" tabindex="0">
            </div>
            <button class="btn-info" id="randomBtn" tabindex="0" style="margin-left:auto;padding:8px 16px;">🎲 Aleatorio</button>
            <div class="user-profile" tabindex="0">MH</div>
        </div>
        ...
        <div class="scroll-area" id="scrollArea">
            <div class="hero" id="hero">
                <div class="hero-content">
                    ...
                    <div class="hero-desc" id="heroDesc">Explora miles de películas seleccionadas especialmente para ti. Usa el control para navegar.</div>
                    <div class="hero-buttons">
                        <button class="btn-play" id="heroPlay" tabindex="0">▶ Reproducir</button>
                        <button class="btn-info" id="heroInfo" tabindex="0">ℹ️ Información</button>
                    </div>
                </div>
            </div>
    ...
    <div class="player" id="player">
        <div class="p-ui" id="pUi">
            ...
            <div class="p-bottom">
                <div class="p-prog" id="pProg" tabindex="0">
                ...
                <div class="p-ctrl">
                    <button class="p-btn" id="pRw" tabindex="0" title="Retroceder 10s">⏪</button>
                    <button class="p-btn main" id="pPp" tabindex="0" title="Play/Pause">▶</button>
                    <button class="p-btn" id="pFw" tabindex="0" title="Adelantar 10s">⏩</button>
                </div>
            </div>
        </div>
...
const el = {
    ...
    heroInfo: $('heroInfo'), randomBtn: $('randomBtn'), loading: $('loading'),
    logo: $('logo'), sidebar: $('sidebar'), topBar: document.querySelector('.top-bar'),
    navHint: $('navHint'), pProg: $('pProg')
};
...
const S = {
    ...
    focusIndex: -1,
    inSearch: false,
    focusGroup: 'nav' // nav, search, hero, grid, player
};
...
function getAllFocusables() {
    ...
    // Sidebar
    els.push(...el.sidebar.querySelectorAll('[tabindex="0"]'));
    // Top bar
    els.push(el.srch, el.randomBtn, document.querySelector('.user-profile'));
    // Hero
    els.push(el.heroPlay, el.heroInfo);
    // Grid
    els.push(...el.grid.querySelectorAll('.card'));
...
function focusNext() {
    ...
    S.focusIndex = (S.focusIndex + 1) % focusables.length;
    setFocus(focusables[S.focusIndex]);
}
...
function focusPrev() {
    ...
    S.focusIndex = (S.focusIndex - 1 + focusables.length) % focusables.length;
    setFocus(focusables[S.focusIndex]);
}
...
function mkCard(m) {
    ...
    const imgHtml = m.poster ? '<img data-src="' + esc(m.poster) + '" alt="' + esc(m.title) + '">' : '<div style="width:100%;height:100%;background:linear-gradient(135deg,#2a2a2a,#1a1a1a)"></div>';
    
    d.innerHTML = imgHtml + '<div class="card-overlay"><div class="card-title">' + esc(m.title) + '</div></div><div class="card-play">▶</div>';
    d.onclick = () => play(m);
...
el.vid.oncanplay = () => el.pStatus.classList.remove('show');
...
el.vid.ondurationchange = () => {
    if (isFinite(el.vid.duration)) el.pDur.textContent = fmt(el.vid.duration);
};
...
el.vid.onprogress = () => {
    try {
        if (el.vid.buffered && el.vid.buffered.length > 0) {
            ...
            el.pBuf.style.width = (bufferedEnd / el.vid.duration * 100) + '%';
        }
    } catch (e) {}
};
...
el.vid.onerror = () => {
    ...
    el.pStatusTxt.textContent = codes[err?.code] || 'Error';
    el.pStatus.classList.add('show');
};
...
el.heroPlay.onclick = () => { if (S.allMovies.length > 0) play(S.allMovies[0]); };
el.heroInfo.onclick = () => alert('Película: ' + el.heroTitle.textContent);
el.randomBtn.onclick = () => { S.currentPage = 0; S.searchQuery = ''; el.srch.value = ''; loadMovies(0, '', true); };
...
document.addEventListener('keydown', e => {
    if (S.view === 'player') {
        ...
        else if (e.key === 'ArrowUp') { e.preventDefault(); el.vid.volume = Math.min(1, el.vid.volume + 0.1); }
        else if (e.key === 'ArrowDown') { e.preventDefault(); el.vid.volume = Math.max(0, el.vid.volume - 0.1); }
        else if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggle(); }
        else if (e.key === 'Escape') closeP();
        return;
    }

    if (S.inSearch && e.key !== 'Escape') return;
    ...
    if (e.key === 'ArrowDown') focusNext();
        ...
        const focusables = getAllFocusables();
        if (focusables[S.focusIndex]) focusables[S.focusIndex].click();
    }
...
el.scrollArea.addEventListener('scroll', () => {
    ...
    if (scrolled > el.scrollArea.scrollHeight - 500) {
        S.currentPage++;
